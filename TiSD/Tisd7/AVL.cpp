...
template<typename T>
void AVL<T>::free_tree(AVL_Node<T> *&tmp)
{
	if (tmp)
	{
		free_tree(tmp->left);
...
template<typename T>
int AVL<T>::height(AVL_Node<T> *t)
{
	return t == NULL ? 0 : t->h;
}

template<typename T>
int AVL<T>::difference_of_h(AVL_Node<T> *t)
{
	return height(t->right) - height(t->left);
}

template<typename T>
int AVL<T>::cnt_value(AVL_Node<T> *t)
{
	return t == NULL ? 0 : t->cnt;
}

template<typename T>
T AVL<T>::key_value(AVL_Node<T> *t)
{
	return t == NULL ? (0) : t->key;/////////что это?////////////////TODO
}

template<typename T>
void AVL<T>::right(AVL_Node<T> *&t)
{
...
template<typename T>
void AVL<T>::balance(AVL_Node<T> *&t) // балансировка узла p
{
	t->h = max(height(t->left), height(t->right)) + 1;
...
template<typename T>
void AVL<T>::add_to_avl(AVL_Node<T> *&t, AVL_Node<T> *tmp)
{
	if (t == NULL)
	{
		t = tmp;
...
template<typename T>
AVL_Node<T> *AVL<T>::removemin(AVL_Node<T> *t)
{
	if (t->left == NULL)
		return t->right;
...
template<typename T>
bool AVL<T>::delete_from_avl(AVL_Node<T> *&t, T x)
{
