...
template<typename T>
HashTableOpen<T>::HashTableOpen(int Start_size)
{
...
template<typename T>
int HashTableOpen<T>::Memory()
{
	return sizeof(ElementTable<T>) * count_of_elem;
}

template<typename T>
void HashTableOpen<T>::New_Table(int new_size)
{
...
template<typename T>
bool HashTableOpen<T>::Insert(T k)
{
...
template<typename T>
bool HashTableOpen<T>::Search(T x)
{
...
template<typename T>
bool HashTableOpen<T>::Delete_element(T k)
{
	unsigned int h = hash_function(k) % tableSize;
	ElementTable<T> *tmp = table[h];
	if (status[h] == 0 || !tmp)
