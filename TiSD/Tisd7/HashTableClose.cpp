...
template<typename T>
HashTableClose<T>::HashTableClose(int Start_size)
{
...
template<typename T>
int HashTableClose<T>::Memory()
{
	return tableSize * sizeof(T);
}

template<typename T>
HashTableClose<T>::~HashTableClose()
{
	delete[] table;
	delete[] status;
}
...
template<typename T>
void HashTableClose<T>::New_Table(int new_size)
{
...
template<typename T>
bool HashTableClose<T>::Insert(T k)
{
...
template<typename T>
bool HashTableClose<T>::Search(T x)
{
...
template<typename T>
bool HashTableClose<T>::Delete_element(T x)
{
	unsigned int h = hash_function(x) * 2 + 1;
	for (int i = 0; i < tableSize; ++i)
	{
